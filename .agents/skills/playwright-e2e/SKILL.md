...
1. **Pre-flight Readiness**:
   - `devenv shell test-e2e` automatically invokes `stack-start` (which ensures PostgreSQL via `devenv shell db-start`, boots `booking_api`, `listing_api`, and `user_api` in background if offline via `devenv shell apis-start`, and boots `web_app_tc` and `web_app_admin_tc` via `devenv shell frontends-start`).
...
2. **Execution Mode Selection (Headless vs. UI)**:
   ...
   - **Interactive UI Mode**: When the user specifies UI mode (e.g., "with UI", "launch UI", "in UI"), append `--ui` or run `devenv shell test-e2e-ui`.
...
3. **Run Test Suites**:
   - **Headless Runs (Default)**:
     - Guest Portal (Chromium):
       ```bash
       devenv shell test-e2e --project=guest-portal-chromium
       ```
     - Admin Portal (Chromium):
       ```bash
       devenv shell test-e2e --project=admin-portal-chromium
       ```
     ...
     - Direct API Sanity:
       ```bash
       devenv shell test-e2e --project=backend-api
       ```

   - **Interactive UI Runs (When User Requests UI)**:
     - Full UI Runner:
       ```bash
       devenv shell test-e2e --ui
       ```
     - Guest Portal in UI:
       ```bash
       devenv shell test-e2e --ui --project=guest-portal-chromium
       ```
     - Admin Portal in UI:
       ```bash
       devenv shell test-e2e --ui --project=admin-portal-chromium
       ```
