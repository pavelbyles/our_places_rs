            ...
            {move || {
                listing_resource.get().map(|res| match res {
                    Ok(details) => {
                        ...
                        view! {
                            <div class="w-full max-w-6xl mx-auto mt-4 mb-20 px-4 flex flex-col gap-6">
                                {carousel_content}
                                
                                <div>
                                    <h1 class="text-4xl font-bold">{listing.name.clone()}</h1>
                                    ...
                                    <div class="text-3xl font-bold mt-4 text-primary">
                                        "$" {listing.price_per_night.map(|p| p.to_string()).unwrap_or_else(|| "0.00".to_string())} <span class="text-lg font-normal text-base-content/70">"/ night"</span>
                                    </div>
                                    
                                    <div class="mt-8">
                                        <h2 class="text-2xl font-semibold mb-3">"About this place"</h2>
                                        <p class="whitespace-pre-line text-lg text-base-content/80 leading-relaxed max-w-4xl">{listing.description.clone().unwrap_or_default()}</p>
                                    </div>

                                    <div class="mt-8 mb-10 flex flex-wrap gap-6 text-base-content/80 text-lg border-y border-base-200 py-6">
