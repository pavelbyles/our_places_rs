...
#[page(POST "/admin/bookings/{code}/messages")]
pub async fn send_booking_message_admin_htmx(
    cx: &Cx,
...
#[page("/admin/bookings/{code}/messages")]
pub async fn booking_messages_admin_page(cx: &Cx) -> Result<impl View> {
    let user = web_app_common_tc::auth::require_admin_auth(cx).await?;
    let code: &str = path_param::<Code>(cx);
    ...
    let booking = bookings
        .into_iter()
        .find(|b| b.confirmation_code.eq_ignore_ascii_case(code) || b.id.to_string() == code)
        .ok_or_redirect("/admin/bookings")?;

    // Fetch user to get guest name
    let users = api
        .get_all_users(Some(1), Some(1000), None)
        .await
        .unwrap_or_default();
    let guest = users.into_iter().find(|u| u.id == booking.guest_id);
    let guest_name = guest
        .map(|u| format!("{} {}", u.first_name, u.last_name))
        .unwrap_or_else(|| "Guest".to_string());

    // Fetch listing details for property name and link
    ...
    Ok(view! {
        <div class="max-w-4xl mx-auto px-4 py-8">
            <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <a href="/admin/bookings" class="btn btn-ghost btn-sm pl-0">"← Back to Bookings"</a>
                    <h1 class="text-3xl font-extrabold tracking-tight mt-2">"Message " (guest_name)</h1>
            ...
            <div class="card bg-base-100 shadow-xl border border-base-200 flex flex-col h-[600px] overflow-hidden">
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-2 bg-base-100/50" id="admin-messages-list">
                    if messages.is_empty() {
                        <div class="text-center text-sm text-base-content/50 my-10 italic">"No messages yet."</div>
                    } else {
                        for m in messages {
                            // In admin panel, Admin/Host is "me" (right aligned)
                            let is_me = m.sender_role == MessageSenderRole::Admin || m.sender_role == MessageSenderRole::Host;
                ...
                if is_cancelled {
                    ...
                    <div class="p-4 border-t border-base-200 bg-base-100">
                        <form
                            hx-post=(format!("/admin/bookings/{}/messages", conf_code))
                            hx-target="#admin-messages-list"
                            hx-swap="beforeend"
                            onsubmit="setTimeout(() => { this.reset(); let list = document.getElementById('admin-messages-list'); list.scrollTop = list.scrollHeight; }, 10);"
                            class="flex gap-3"
                        >
                            <input
                                type="text"
